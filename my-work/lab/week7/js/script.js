...
let removeUpdate = () => {
  theSituation = graphGroup.selectAll('.datapoint').data(data, d => d.key);
...
function removeAndAdd() {
  removeDatapoints(Math.floor(Math.random()*5));
  removeUpdate();
  setTimeout(() => {
    addDatapoints(Math.floor(Math.random()*5));
    update();
  }, 1000);
}
...
document.getElementById("buttonF").addEventListener("click", () => {
  data.forEach((it, index) => {
    data[index].value = it.value * Math.random();
  })
  theSituation = graphGroup.selectAll(".datapoint").data(data);
  update();
});