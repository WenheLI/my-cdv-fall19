...
var main = function main() {
  return __awaiter(void 0, void 0, void 0, function () {
    var w, h, padding, atarri, nextButton, displayDiv, statsButton, textArea, viz, data, gameData, years, salesData, selectedData, xDomain, xScale, xAxis, xAxisGroup, yMax, yDomain, yScale, yAxis, yAxisGroup, lineMaker;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          ...
          padding = 90;
          atarri = document.getElementById('shock');
          atarri.addEventListener('click', function (e) {
            document.getElementsByClassName('shader')[0].className = 'shader show';
          });
          document.getElementById('atari-close').addEventListener('click', function () {
            document.getElementsByClassName('shader')[0].className = 'shader hide';
          });
          nextButton = document.getElementById('next-button-img');
          ...
          statsButton = document.getElementById('stats-button-img');
          ...
          viz = d3.select('#container').append('svg').style('width', w).style('height', h).style('background', 'aliceblue');
          return [4
          /*yield*/
          , d3.csv('./videogames.csv')];

        case 1:
          data = _a.sent();
          data = data.filter(function (it) {
            return it['yearOfRelease'] !== 'N/A';
          }).filter(function (it) {
            return it['yearOfRelease'] !== '2017';
          }).filter(function (it) {
            return it['yearOfRelease'] !== '2016';
          }).filter(function (it) {
            return it['yearOfRelease'] !== '2020';
          });
          gameData = data.map(function (it) {
            var d = {};
            var strings = ["name", "platform", "publisher", "developer", "rating", "genre"];
            strings.forEach(function (name) {
              d[name] = it[name];
            });
            var floats = ["EUSales", "JPSales", "NASales", "OtherSales", "GlobalSales", "criticScore", "userScore", "userCount", "criticCount"];
            d.yearOfRelease = d3.timeParse('%Y')(it.yearOfRelease);
            floats.forEach(function (float) {
              var temp = parseFloat(it[float]);
              if (!isNaN(temp)) d[float] = temp;else d[float] = -1;
            });
            return d;
          });
          years = Array.from(new Set(data.map(function (it) {
            return it.yearOfRelease;
          })));
          salesData = years.map(function (year) {
            var currYears = d3.timeParse('%Y')(year).getTime();
            var platforms = Array.from(new Set(gameData.filter(function (it) {
              return it.yearOfRelease.getTime() === currYears;
            }).map(function (it) {
              return it.platform;
            })));
            return {
              year: d3.timeParse('%Y')(year),
              sales: sum(gameData.filter(function (it) {
                return it.yearOfRelease.getTime() === currYears;
              }).map(function (it) {
                return it.GlobalSales;
              })),
              platformSales: platforms.map(function (it) {
                var sumSales = sum(gameData.filter(function (game) {
                  return game.platform === it;
                }).map(function (it) {
                  return it.GlobalSales;
                }));
                var o = {};
          ...
          selectedData = salesData.filter(function (it) {
            return it.year.getFullYear() <= 1995;
          });
          xDomain = d3.extent(selectedData, function (d) {
            return d.year;
          });
          xScale = d3.scaleTime().domain(xDomain).range([padding, w - padding]);
          xAxis = d3.axisBottom(xScale);
          xAxisGroup = viz.append("g").attr("class", "xaxisgroup").attr("transform", "translate(0," + (h - padding) + ")");
          xAxisGroup.call(xAxis);
          yMax = d3.max(selectedData, function (d) {
            return d.sales;
          });
          yDomain = [0, yMax];
          yScale = d3.scaleLinear().domain(yDomain).range([h - padding, padding]);
          yAxis = d3.axisLeft(yScale);
          yAxisGroup = viz.append("g").attr("class", "yaxisgroup").attr("transform", "translate(" + padding / 2 + ",0)");
          yAxisGroup.call(yAxis);
          lineMaker = d3.line().x(function (d) {
            return xScale(d.year);
          }).y(function (d) {
            return yScale(d.sales);
          });
          viz.selectAll(".global").data([selectedData]).enter().append("path").attr('class', 'line global').attr("d", lineMaker).attr('stroke', 'black').attr('stroke-width', '5px').style('fill', 'none').style('opacity', 1); // const length = d3.selectAll('.global').node().getTotalLength();
          // console.log(length)

          viz.selectAll('.point').data(salesData).enter().append('circle').attr('class', 'point').attr('cx', function (d) {
            return xScale(d.year);
          }).attr('cy', function (d) {
            return yScale(d.sales);
          }).attr('r', function (d) {
            return 5;
          }).style('fill', '#c71585').on('mouseenter', function (d) {
            viz.append('line').attr('class', 'animatedLine').style("stroke", "#c71585").style("stroke-weight", 30).attr('x1', xScale(d.year)).attr('y1', yScale(d.sales)).attr('x2', xScale(d.year)).attr('y2', yScale(d.sales)).transition().duration(1000).attr('y2', yScale(0)); // textArea.innerHTML = '';
            // textArea.innerHTML = `<p>This is ${d.year.getFullYear()} </p> <br > <p> The global sales are ${d.sales.toFixed(2)} millions</p> <br >`

            d3.select(this).append('text');
            d3.select(this).transition().style('fill', 'orange').attr('r', 10);
          }).on('mouseleave', function (d) {
            d3.select(this).transition().style('fill', '#c71585').attr('r', 5); //  textArea.innerHTML = ``;

            viz.selectAll('.selectedArea').remove();
            viz.selectAll('.animatedLine').transition().duration(1000).attr('y2', yScale(d.sales)).remove();
          });
          return [2
          /*return*/
          ];
...
//# sourceMappingURL=/console-game-viz.77de5100.js.map